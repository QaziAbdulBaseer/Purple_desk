...
from myapp.serializers import MembershipSerializer
...
async def create_membership(request, location_id):
    """Create a new membership for a specific location"""
    try:
...
async def get_memberships(request, location_id):
    """Get all memberships for a specific location"""
    try:
        user, error_response = await require_admin(request)
        if error_response:
            return error_response
        
        memberships = await sync_to_async(lambda: list(
            Membership.objects.filter(location_id=location_id)
        ))()
...
async def get_membership(request, location_id, pk):
    """Get a specific membership by its primary key and location"""
    try:
        user, error_response = await require_admin(request)
        if error_response:
            return error_response
        ...
        membership = await sync_to_async(Membership.objects.get)(
            membership_id=pk, location_id=location_id
        )
...
async def update_membership(request, location_id, pk):
    """Update a membership"""
    try:
        user, error_response = await require_admin(request)
        if error_response:
...
async def delete_membership(request, location_id, pk):
    """Delete a membership"""
    try:
        user, error_response = await require_admin(request)
        if error_response:
